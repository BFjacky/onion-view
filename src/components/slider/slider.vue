...
<template>
  <div class='slider-container' :class = 'basicdata.containerClass'>
    ...
    <template v-if="!sliderinit.loop">
        <slot v-for="item in pages" v-bind:name="item.name">   
        </slot>
    </template>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    styleobj: function() {
      ...
      style["transform"] =
        "translate3D(" +
        this.basicdata.poswidth +
        "," +
        this.basicdata.posheight +
        ",0)";
      style[this.basicdata.prefixes.transition + "TimingFunction"] = "ease";
      style[this.basicdata.prefixes.transition + "Duration"] =
        (this.basicdata.animation ? this.sliderinit.slideSpeed || 300 : 0) +
        "ms";
      return style;
    },
    // pagenum滑动数
    ...
    currentWidth: {
      get: function() {
        ...
        if (this.sliderinit.loop) {
          if (this.sliderinit.infinite) {
            lastPage =
              ...
              (this.sliderinit.infinite <= this.pages.length
                ? this.sliderinit.infinite
                : this.pages.length);
          } else {
        ...
        for (let item in this.$el.children) {
          if (/slider-wrapper/gi.test(this.$el.children[item].className)) {
  ...
  methods: {
    ...
    swipeEnd(e) {
      ...
      if (deltaTime > this.sliderinit.thresholdTime) {
        ...
        if (
          deltaX > this.sliderinit.thresholdDistance &&
          Math.abs(deltaY) < this.sliderinit.thresholdDistance
        ) {
          // swipe right
          this.pre();
          return;
        ...
        if (
          deltaY > this.sliderinit.thresholdDistance &&
          Math.abs(deltaX) < this.sliderinit.thresholdDistance
        ) {
          // swipe right
          this.pre();
          return;
    ...
    next() {
      if (this.sliderinit.currentPage < this.pagenums - 1) {
        ...
        this.sliderinit.loop &&
        this.sliderinit.currentPage === this.pagenums - 1
      ) {
        this.sliderinit.currentPage += this.sliderinit.slidesToScroll || 1;
        this.basicdata.transitionEnding = true;
        this.slide();
    ...
    slide(pagenum, type) {
      ...
      // 广播事件
      if (
        that.sliderinit.currentPage < 0 ||
        that.sliderinit.currentPage >= that.pagenums
      ) {
        return;
      }
        ...
        begin: function(that) {
          if (that.sliderinit.autoplay) {
            ...
            that.basicdata.setIntervalid = setInterval(function() {
              that.next();
              if (
                that.sliderinit.currentPage === that.pagenums - 1 &&
                !that.sliderinit.loop
              ) {
    ...
    onTransitionEnd() {
      if (this.sliderinit.loop) {
        ...
        if (this.sliderinit.currentPage < 0) {
          this.slide(
            this.pagenums + this.sliderinit.currentPage,
            "animationnone"
          );
        } else if (this.sliderinit.currentPage >= this.pagenums) {
          this.slide(
            0 + this.sliderinit.currentPage - this.pagenums,
            "animationnone"
          );
        }
      }
